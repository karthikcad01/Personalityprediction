...
class FBWebScraper():
    ...
    def __init__(self, my_email, my_password, my_profile_url, my_name, statuses=3, scroll_time=15, browser='Chrome'):
    ...
    def create_friends_dict(self):
        ...
        self.fb_statuses.insert({
            ...
            'html': html,
            'url': self.my_profile_url
        }, check_keys=False)

    def my_status(self):
        person_dict = self.fb_statuses.find_one({"url": self.my_profile_url})
        self.browser.get(self.my_profile_url)
        self.browser.find_element_by_css_selector(f'a[href="{self.my_profile_url}"]').click()
        time.sleep(self.scroll_time)
        if person_dict == None:
            statuses_dict = {}
        ...
        while len(statuses_dict.items()) < self.number_of_statuses:
            ...
            for post in posts:
                try:
                    post_time_element = post.find_element_by_tag_name('abbr')
                    post_time = post_time_element.get_attribute('title')
                    post_context = self.my_name
                    print(post_context)
                    try:
